...
public static class GetPrice_Transpliters
{
    ...
    {
        var price = UIController.GetPrice(item);
    ...
    public static IEnumerable<CodeInstruction> RepalceGetPrice(IEnumerable<CodeInstruction> instructions)
    {
        ...
        foreach (var instruction in instructions)
        {
            if (instruction.operand is MethodInfo operand)
            {
                if (operand == method1)
                {
                    yield return new CodeInstruction(OpCodes.Call, ((Func<Shop, Shop.ShopType>)GetShopType).Method);
                }
                else if (operand == method2)
                {
                    yield return new CodeInstruction(OpCodes.Call, ((Func<Shop, ShopDefItem, Shop.PurchaseType, Shop.ShopType, int>)GetPrice).Method);
                }
                else
                {
