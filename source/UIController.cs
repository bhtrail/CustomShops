...
public static class UIController
{
    ...
    internal static void InitShopWindow(SG_Shop_Screen shopScreen)
    {
        Control.LogDebug(DInfo.ShopInterface, "Enter Shop Screen");
        if (shopScreen != ShopScreen)
        {
            SetupButtons(shopScreen);
        }
        ...
        foreach (var button in Buttons.Values)
        {
            try
            {
                ...
                if (shop.Exists)
                {
                    ...
                    if (shop is ISpriteIcon spriteIcon)
                    {
                        Control.LogDebug(DInfo.ShopInterface, $"--- sprite image - set");
                        button.Button.SetImageAndText(spriteIcon.Sprite, shop.TabText);
                    ...
                    else if (shop is ITextIcon textIcon)
                    {
                        var id = textIcon.SpriteID;
    ...
    public static int GetPrice(ShopDefItem item)
    {
        ...
        int price = 1;
        float discount = 1;
        if (ActiveShop is IDefaultPrice)
        {
        ...
        else if (ActiveShop is ICustomPrice customPrice)
        {
            price = customPrice.GetPrice(titem);
        }
        else
        {
        ...
        else if (ActiveShop is IDiscountFromFaction faction_discount)
        {
            discount = 1 + Control.State.Sim.GetReputationShopAdjustment(faction_discount.RelatedFaction);
        }
        else if (ActiveShop is INoDiscount)
        {
            discount = 1;
        }
        else if (ActiveShop is ICustomDiscount cdisc)
        {
    ...
    public static bool DefaultPurchase(IListShop shop, ShopDefItem item, int quantity)
    {
        var list = shop.Items;
    ...
    private static bool BuyItem(ShopDefItem item, int quantity)
    {
        ...
        if (ActiveShop is ICustomPurchase customPurchase)
        {
            return customPurchase.Purchase(item, quantity);
        }
        else if (ActiveShop is IDefaultShop defaultShop)
        {
            Control.LogDebug(DInfo.ShopActions, $"---- IDefaultShop");
            return DefaultPurshase(defaultShop, item, quantity);
        }
        else if (ActiveShop is IListShop listShop)
        {
            Control.LogDebug(DInfo.ShopActions, $"---- IListShop");
            return DefaultPurchase(listShop, item, quantity);
        }
